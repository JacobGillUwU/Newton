...
class MagicNewtonAPIClient {
        ...
        try {
            ...
            if (userQuestsResult.success) {
                ...
                if (completedQuests.length > 0) {
                    ...
                    if (hoursDiff >= 24) {
                        this.log(`Đã quá 24 giờ kể từ lần roll cuối, tiến hành roll mới...`, 'info');
                    } else {
            ...
            if (error.response?.status === 400 && error.response?.data?.message === 'Quest already completed') {
                ...
                if (userQuestsResult.success) {
                    ...
                    if (completedQuests.length > 0) {
                        ...
                        if (hoursDiff >= 24) {
                            this.log(`Đã quá 24 giờ kể từ lần roll cuối, thử roll lại...`, 'info');
    ...
    calculateWaitTime(accountResults) {
        ...
        this.log(`Tài khoản có thời gian chờ lâu nhất: ${latestResult.email}`, 'info');
        return waitSeconds;
    }
    ...
    async processAccount(token, proxyIndex) {
        try {
            ...
            this.log(`Xử lý tài khoản với IP: ${proxyIP}`, 'custom');
    ...
    async main() {
        try {
            this.log(`Đọc được ${this.proxies.length} proxy`, 'info');
            ...
            this.log(`Đọc được ${tokens.length} tài khoản`, 'info');
            ...
            while (true) {
                ...
                for (let i = 0; i < tokens.length; i++) {
                    ...
                    console.log(`========== Xử lý tài khoản ${i + 1} | ip: ${proxyIP} ==========`);
                ...
                this.log(`Thời gian bắt đầu chạy tiếp theo: ${waitTimeFormatted}`, 'custom');
